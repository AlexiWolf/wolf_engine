            ...
            match event {
                ...
                WinitEvent::DeviceEvent { event, .. } => {
                    if let Some(input) = event.to_input() {
                        (event_handler)(Event::Input(None, input), &context);
                    }
                }
                WinitEvent::WindowEvent {
                    ...
                    if let Some(input) = window_event.to_input() {
                        (event_handler)(Event::Input(Some(uuid), input), &context);
                    }
                    match window_event {
                        WinitWindowEvent::RedrawRequested => {
                            (event_handler)(Event::RedrawRequested(uuid), &context)
                        }
                        WinitWindowEvent::Resized(size) => {
                            (event_handler)(Event::Resized(uuid, size.width, size.height), &context)
                        }
                        WinitWindowEvent::CloseRequested => {
                            (event_handler)(Event::Closed(uuid), &context)
                        }
                        _ => (),
                    }
                }
